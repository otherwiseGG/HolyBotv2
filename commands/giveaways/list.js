...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        const msg = await message.reply({
            embeds: [new Discord.MessageEmbed().setDescription("Choose an option in the select menu to get started!").setColor("#2F3136").setTimestamp()],
            components: [row]
        })
        ...
        collector.on("collect", async (i) => {
            ...
            if (val == "guildReq") {
                if (!giveaways.some(e => e.extraData)) return msg.edit({
                    content: '💥 No Giveaways To Be Displayed',
                    embeds: []
